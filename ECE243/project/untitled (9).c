...
int main(void)
{
	...
	highlightNextMoves(GameBoard[0][8], RED);
	drawGameBoard();
}
...
bool highlightNextMove(struct Box b, int player){
	...
	for(int i = 0; i<4; i++){
		...
		if(rowAndColIsValid(nextRow, nextCol){
			if(nextBoxIsValid(GameBoard[nextRow][nextCol])){
    ...
    for(int i = 4; i<8; i++){
		...
		if(rowAndColIsValid(nextnextRow, nextnextCol){
			if(nextBoxIsValid(GameBoard[nextnextRow][nextnextCol])){
				if(selectedBoxIsValid(GameBoard[nextRow][nextCol])){
