...
int main(void)
{
	...
	printf("user select box r/c\n");
	scanf("%d%d", &userRow, &userCol);
	
	while(!gameEnded){
		
		//check selected move is valid 
		if(rowAndColIsValid(userRow, userCol)){
			if(selectedBoxIsValid(GameBoard[userRow][userCol], userPlayer)){
				
				//highlight next move
				highlightNextMove(GameBoard[userRow][userCol], userPlayer);
				
				//ask for next move
				printf("user next box r/c\n");
				scanf("%d%d", &userNextRow, &userNextCol);
				
				//check next move is valid
				if(rowAndColIsValid(userNextRow, userNextCol)){
					for(int i = 0; i<8; i++){
						if(userRow == NextRowMoves[i] && userCol == NextColMoves[i]){
							userInputIsValid = true;
							changeGameBoard(userRow, userCol, EMPTY);
							changeGameBoard(userNextRow, userNextCol, userPlayer);
						}
					}
				}
			}
	    ...
	    //check user change 
	    if(userInputIsValid){
			userPlayer = userPlayer*-1;
		}else{
			printf("incorrect input, select again \n");
		}	
...
bool gameEnd(){
	...
	bool redBoxHasDifferentPlayer = false;
	bool greenBoxHasDifferentPlayer = false;
	bool redBoxHasEmpty = false;
	bool greenBoxHasEmpty = false;
	
	//iterate through every REDBOX and GREENBOX on the gameboard
	for (int r = 0; r < ROW; r++) {
		for (int c = 0; c < COL; c++) {
						...
						if (((ROW - 1) / 2 - c) % 2 == 0) {
							...
							if (c <= 3) {
								if(GameBoard[r][c].state == EMPTY){
									...
									redBoxHasDifferentPlayer = true;
								}
							}
							
							if (c >= 13) {
								if(GameBoard[r][c].state == EMPTY){
									greenBoxHasEmpty = true;
								}else if(GameBoard[r][c].state == RED){
									greenBoxHasDifferentPlayer = true;
								}
							}
						}
					}
	...
	//tie
	if(!greenBoxHasEmpty && greenBoxHasDifferentPlayer){
		if(!redBoxHasEmpty && redBoxHasDifferentPlayer){
			Winner = TIE;
			return true;
		}
	}
