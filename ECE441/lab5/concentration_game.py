# -*- coding: utf-8 -*-
"""
Created on Thu Aug 17 15:54:32 2023
...
def save_text_to_file(text, filename):
    # Open the file in append mode ('a') to add content without overwriting the file
    with open(filename, 'a') as file:
        file.write(text + '\n')  # Write the text to the file with a newline for each entry

def animate(frame):
    alpha_beta_ratio = calculate_alpha_beta_ratio()

    text_to_save = str(alpha_beta_ratio)  # Convert alpha_beta_ratio to string
    
    # Save the text to file at each frame
...
filename = "output2frontelectrode.txt"
